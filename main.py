...
# Trains BCQ offline
def train_BCQGAN(state_dim, action_dim, max_action, device, args):
	# For saving files
	setting = f"{args.env}_{args.seed}"
	buffer_name = f"{args.buffer_name}_{setting}"
	ac_lr = args.ac_lr
	...
	use_sngan = bool(args.use_sngan)
	use_ac = bool(args.use_ac)
	use_max_a = bool(args.use_max_a)
	use_wgan = bool(args.use_wgan)
	wgan_d_iter = args.wgan_d_iter

	# Initialize policy
	policy = BCQGAN.BCQGAN(state_dim, action_dim, max_action, device, args.discount, args.tau, args.lmbda, args.phi,
						   ac_lr, gan_lr, hidden_size, use_sngan, use_ac, use_max_a, use_wgan, wgan_d_iter)
	...
	while training_iters < args.max_timesteps: 
		...
		evaluations.append(eval_policy(policy, args.env, args.seed))
		np.save(f"./results/BCQGAN_{setting}", evaluations)
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
	...
	print("---------------------------------------")
	print(f"Evaluation over {eval_episodes} episodes: {avg_reward:.3f} len: {avg_len:.3f}")
	print("---------------------------------------")
...
if __name__ == "__main__":
	...
	parser.add_argument("--ac_lr", default=0.001, type=float)  		# A and C learning rate
	parser.add_argument("--gan_lr", default=0.001, type=float)  	# G and D learning rate
	parser.add_argument("--hidden_size", default=64, type=int)  	# NN hidden layer size
	parser.add_argument("--use_sngan", default=0, type=int)  		# use SN-GAN in discriminator
	parser.add_argument("--use_ac", default=1, type=int)  			# use Actor-Critic architecture
	parser.add_argument("--use_max_a", default=1, type=int)  		# use max action value at action selection time
	parser.add_argument("--use_wgan", default=0, type=int)  		# use WGAN loss
	parser.add_argument("--wgan_d_iter", default=5, type=int)  		# train WGAN Discriminator this many times per Generator
	...
	if args.train_behavioral:
		...
		print(f"Setting: Training BCQGAN, Env: {args.env}, Seed: {args.seed}")
	print("---------------------------------------")
	...
	if args.train_behavioral or args.generate_buffer:
		...
		train_BCQGAN(state_dim, action_dim, max_action, device, args)
